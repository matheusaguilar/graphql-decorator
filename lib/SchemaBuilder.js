...
class SchemaBuilder {
    ...
    registerModels(...models) {
        for (const model of models) {
            ...
            console.log(Reflect.hasMetadata(Decorators_1.GRAPHQL_MODEL_ENTITY, modelInstance));
            if (Reflect.hasMetadata(Decorators_1.GRAPHQL_MODEL_ENTITY, modelInstance)) {
                const modelType = GraphQlModelCreator_1.getGraphQLModel(modelInstance);
            ...
            for (const method of Object.getOwnPropertyNames(Object.getPrototypeOf(resolver))) {
                if (Reflect.hasMetadata(Decorators_1.GRAPHQL_RESOLVER_QUERY, resolver, method)) {
                    ...
                    if (Reflect.hasMetadata(Decorators_1.GRAPHQL_RESOLVER_RETURN, resolver, method)) {
                        queryFields[queryName].type = Reflect.getMetadata(Decorators_1.GRAPHQL_RESOLVER_RETURN, resolver, method);
                    }
                    else {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const method of Object.getOwnPropertyNames(Object.getPrototypeOf(resolver))) {
                if (Reflect.hasMetadata(Decorators_1.GRAPHQL_RESOLVER_MUTATION, resolver, method)) {
                    ...
                    // add return type
                    if (Reflect.hasMetadata(Decorators_1.GRAPHQL_RESOLVER_RETURN, resolver, method)) {
                        mutationFields[queryName].type = Reflect.getMetadata(Decorators_1.GRAPHQL_RESOLVER_RETURN, resolver, method);
                    }
                    else {
                        console.error(`GraphQL: Schema: Error at ${resolver.constructor.name} in method ${method}, no returnType defined.`);
                    }
    ...
    getArgsTypes(resolver, method, argNames) {
        ...
        argNames.forEach((arg, index) => {
            if (pArgs[index].name.toLowerCase() !== 'ResContext'.toLowerCase()) {
                // not ResContext
                const modelInputType = GraphQlModelCreator_1.getGraphQLModel(new pArgs[index]());
