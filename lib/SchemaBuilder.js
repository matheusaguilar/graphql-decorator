...
class SchemaBuilder {
     ...
     * @param argsLength
     */
    getResolverArgsArray(hasArgs, args, resolver, method, context, argsLength) {
        const argsAsArray = [];
        if (hasArgs) {
                ...
                if (Reflect.hasMetadata(Decorators_1.GRAPHQL_RESOLVER_QUERY, resolver, method)) {
                    ...
                    queryFields[queryName].resolve = (_, args, context) => {
                        if (this.validateAuthFunctions(resolver, method, context)) {
                            const argsAsArray = this.getResolverArgsArray(hasArgs, args, resolver, method, context, argsLength);
     ...
     */
    validateAuthFunctions(resolver, method, context) {
        let count = 0;
        const funcs = [];
        if (Reflect.hasMetadata(Decorators_1.GRAPHQL_RESOLVER_AUTH, resolver, method)) {
            const next = Reflect.getMetadata(Decorators_1.GRAPHQL_RESOLVER_AUTH, resolver, method);
            if (next) {
    ...
    getArgsTypes(resolver, method, argNames) {
        const args = {};
        const pArgs = Reflect.getMetadata('design:paramtypes', resolver, method);
        const options = Reflect.getMetadata(Decorators_1.GRAPHQL_RESOLVER_PARAM, resolver, method);
        argNames.forEach((arg, index) => {
            if (pArgs[index].name.toLowerCase() !== 'ResContext'.toLowerCase()) {
                let optionsArg = null;
                let modelType = null;
                let isArray = false;
                options === null || options === void 0 ? void 0 : options.forEach((opt) => {
                    if (opt.index === index) {
                ...
                const type = GraphQlType_1.getGraphQLBasicType(modelType.name);
                if (type) {
                    modelType = type;
                }
                else {
                    const modelInputType = GraphQlModelCreator_1.getGraphQLModel(new modelType(), this.resolverModelFunction);
